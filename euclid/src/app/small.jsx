...
import {MenuOutlined,SendOutlined,PlusOutlined,LoadingOutlined,DeleteOutlined, UserOutlined, RocketOutlined, SettingOutlined, QuestionCircleOutlined,PoweroffOutlined} from '@ant-design/icons';
...
const Small = (props) => {
  ...
  return (
    <div style={styles.container}>
      <Row>
        <Drawer title="Menu" placement="left" onClose={showmenu} open={main}>
            ...
            {menus.map((item,index)=>(
              <Item key={index} active>
                <Space>
                  ...
                  <Button type="link" onClick={()=>deli(item.chat_id)} icon={<DeleteOutlined/>}></Button>
                </Space>
              </Item>
            ))}
            </div>
        ...
        <Col span={24}>
          <div>
            <Space>
            <Button type='text' onClick={showmenu}><MenuOutlined/></Button>
      ...
      <FloatButton type='primary' icon={<PlusOutlined/>} onClick={showModal} style={{bottom:100,right:20}}/>
      <Modal title="New Workspace" open={visible} onCancel={showModal} onOk={showModal}>
        <Form layout="vertical" onFinish={Add}>
          ...
          <Form.Item>
            <Button type="primary" htmlType="submit">{loader}Add Workspaces</Button>
          </Form.Item>
        </Form>
      </Modal>
